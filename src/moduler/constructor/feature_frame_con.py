...
class FeatureFrame3dConstructor(Moduler):
                ...
                for tlPptLine in rTlPptFile:
                    ...
                    with open(aggCdrFilePath, "r") as rAggCdrFile:
                        for aggCdrLine in rAggCdrFile:
                            ...
                            assert self.aggregateTimeUnit == conf.AggregateCdrDict.AggregateTimeUnit.HOUR_1
                            assert len(startTime) == 10
                            date = startTime[:8]
                            hour = startTime[8:10]
                    ...
                    for date in joinDictBy_date:
                        ...
                        with open(joinFilePathBy_date, "w") as wJoinFile:
                            ...
                            wJoinFile.write(
                                conf.ROW_SEPERATOR.join([joinLinesBy_hour[hour] for hour in sortedHours]))
    ...
    def __initSharedZeroFfAsLine(self):
        ...
        self.__sharedZeroFfAsLine = conf.COL_SEPERATOR.join(
            map(lambda _: "0", range(len(self.ffFirstRowFmt) * conf.FeatureFrame3dDict.COPY_CNT)))
